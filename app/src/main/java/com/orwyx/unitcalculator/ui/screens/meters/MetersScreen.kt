...
fun MetersScreen(
    ...
    // Billing cycle start — used to restrict closing-date calendar to valid range
    val cycleStartDate = remember(state.settings.readingDate) {
        BillingCycle.of(state.settings.readingDate).start
    }
                                ...
                                while (true) {
                                    ...
                                    detectDragGestures(
                                        onDrag = { change, dragAmount ->
                                            ...
                                            when {
                                                accumulated > threshold  -> { viewModel.moveDown(meter.id); accumulated = 0f }
                                                accumulated < -threshold -> { viewModel.moveUp(meter.id);   accumulated = 0f }
                                            }
                                        }
                                    )
                                }
